...
# ---------------------------------------------------------------------------
# Identity pre-rotation (KERI-style) validator
#
# Adversarial validator for KERI-style pre-rotation identity traces: catches
# post-rotation forgery, backdating, and rotation hijack (an attacker holding
# the current key rotates to a non-committed successor), and verifies the
# victim recovers with the genuinely pre-committed key. Window semantics use
# their own _Window/_tick/_payload, kept independent of the merged rotation
# helpers above so the two spec-mandated checks stay visible.
# ---------------------------------------------------------------------------
...
def _result(passed: bool, detail: str) -> ValidationResult:
    """Build a pre-rotation ``ValidationResult``.
