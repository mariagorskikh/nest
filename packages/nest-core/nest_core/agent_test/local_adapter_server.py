...
"""Reusable literal-loopback server for the frozen Generation 1 adapter profile."""
...
class AdapterDecisionContext:
    """Identifiers fixed by the authenticated, validated driver request."""
...
DecisionHook = Callable[[AdapterDecisionContext, Mapping[str, object]], Mapping[str, object]]
...
class _LocalAdapterServer(ThreadingHTTPServer):
    daemon_threads = True
    ...
    def __init__(
        ...
        decide_intent: DecisionHook,
    ) -> None:
        super().__init__(address, _LocalAdapterHandler)
    ...
    def shutdown(self) -> None:
        """Stop serving and release the managed loopback listener."""
        try:
            super().shutdown()
        finally:
            self.server_close()
    ...
    def decide(self, request: dict[str, Any], request_digest: str) -> tuple[int, bytes]:
        ...
        if context is None:
            return 422, self.error_payload("SCHEMA_INVALID", "Request schema is invalid")
        ...
        with self._state_lock:
            ...
            observation = cast("dict[str, Any]", request["observation"])
            ...
            try:
                intent = (
                    ...
                    if kind == "stop"
                        ...
                        self._decide_intent(
                            AdapterDecisionContext(run_id, event_id, sequence), observation
                        )
                    )
                )
...
class _LocalAdapterHandler(BaseHTTPRequestHandler):
    protocol_version = "HTTP/1.1"

    @property
    def adapter(self) -> _LocalAdapterServer:
        assert isinstance(self.server, _LocalAdapterServer)
        return self.server
    ...
    def do_GET(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler contract
        ...
        if self.path != "/town-driver/1/ready":
            self._send(404, self.adapter.error_payload("SCHEMA_INVALID", "Request rejected"))
            return
        if not self._common_headers_are_valid():
            self._send(
                422,
                self.adapter.error_payload(
                    "UNSUPPORTED_CONTRACT", "Driver contract is unsupported"
                ),
            )
            return
    ...
    def do_POST(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler contract
        ...
        if (
            ...
            self._send(
                422, self.adapter.error_payload("SCHEMA_INVALID", "Request framing is invalid")
            )
            return
        try:
            parsed_value: object = json.loads(body)
        except (json.JSONDecodeError, UnicodeDecodeError):
            parsed_value = None
        if not isinstance(parsed_value, dict):
            self._send(
                422, self.adapter.error_payload("SCHEMA_INVALID", "Request schema is invalid")
            )
            return
        ...
        try:
            ...
            run_id, event_id, sequence = context
            status = 500
            response = self.adapter.error_payload(
                "ADAPTER_INTERNAL",
                "Adapter decision failed",
                run_id=run_id,
                event_id=event_id,
    ...
    def _read_body(self) -> bytes | None:
        ...
        if length_text is None or not length_text.isdecimal():
            self._send(
                422, self.adapter.error_payload("SCHEMA_INVALID", "Content length is invalid")
            )
            return None
        length = int(length_text)
        if length > MAX_BODY_BYTES:
            self._send(
                413, self.adapter.error_payload("BODY_TOO_LARGE", "Request body is too large")
            )
            return None
...
def _validated_kind(request: dict[str, Any]) -> str | None:
    ...
    if set(profile) != {"id", "version", "digest"}:
        return None
...
def _validated_capability_kind(request: dict[str, Any]) -> str | None:
    if request["participant"] != {"id": "provider-0", "role": "provider"}:
        return None
    observation_value = request["observation"]
    if not isinstance(observation_value, dict):
        return None
    observation = cast("dict[str, Any]", observation_value)
...
def create_local_adapter_server(
    *,
    token: str,
    adapter_instance_id: str,
    decide_intent: DecisionHook,
    ...
    """Create the fixed-profile adapter server bound only to literal loopback."""
    is_valid_token = False
    token_digest: bytes | None = None
    try:
        is_valid_token = type(token) is str and _TOKEN_RE.fullmatch(token) is not None
        if is_valid_token:
            token_digest = hashlib.sha256(token.encode("ascii")).digest()
    finally:
        del token
    if not is_valid_token or token_digest is None:
        raise ValueError("token must contain exactly 64 lowercase hexadecimal characters")
    if type(port) is not int or not 0 <= port <= 65535:
        raise ValueError("port must be an integer from 0 through 65535")
    if type(adapter_instance_id) is not str:
        raise ValueError("adapter_instance_id must be a string")
    if not callable(decide_intent):
        raise ValueError("decide_intent must be callable")
    return _LocalAdapterServer(
        ("127.0.0.1", port), token_digest, adapter_instance_id, decide_intent
    )
