...
## Signature-gated plugin: `verified`

`verified` is a dictionary-backed registry that verifies an identity signature
when an `AgentCard` is registered. It rejects unsigned cards, signer/agent ID
mismatches, and cards changed after signing. The `registry_integrity` scenario
compares it with `in_memory` under impersonation, unsigned-registration, and
post-signing mutation attacks.

`verified` is not a replicated registry. It does not inspect gossip traffic,
detect signed equivocation, or mitigate eclipse attacks. Its constructor needs
an `Identity` verifier; the scenario provisions that dependency explicitly.

Source:
[`nest_plugins_reference/registry/verified.py`](../../packages/nest-plugins-reference/nest_plugins_reference/registry/verified.py).
...
1. **Signed cards, re-verified on every gossip hop, not just at
   ...
   extends the registration-only `verified` plugin: `verified` has no
   replication path, while `byzantine_gossip` authenticates writes as they
   move between registry instances.
2. **Signed-equivocation detection + permanent quarantine.** A byzantine
   ...
   byzantine. Registration-only signature checks cannot catch this case.
   The publisher is quarantined on the spot -- evicted from the local view,
